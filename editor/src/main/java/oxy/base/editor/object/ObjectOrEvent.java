    ...
    public float get(long l, ObjectTransform transform) {
        ...
        if (kv == null) {
            ...
            if (!keyframes.isEmpty()) {
                long youngestFrameTime = Long.MAX_VALUE;
                ...
                for (Map.Entry<Long, KeyframeValue> entry : keyframes.entrySet()) {
                    if (entry.getKey() < youngestFrameTime) {
                        youngestFrameTime = entry.getKey();
                    }
                ...
                if (closeAheadEntry != null && closeAhead == youngestFrameTime) {
                    float defaultV = keyframe == null ? transform.defaultValue() : keyframe.value();

                    final KeyframeValue.Keyframe ahead = closeAheadEntry.transformations().get(transform);

                    float ratio = (float) (l - start) / (closeAhead - start);
                    if (ahead.easing() == null) {
                        return ratio >= 0.999f ? ahead.value() : defaultV;
                    }
                    ...
                    return defaultV + (ahead.value() - defaultV) * ratio;
                }
            }
