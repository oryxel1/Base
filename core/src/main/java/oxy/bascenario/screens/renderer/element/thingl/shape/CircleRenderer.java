...
public class CircleRenderer extends ThinGLElementRenderer<Circle> {
    ...
    protected void renderThinGL() {
        if (element.outlineOnly()) {
            ThinGL.renderer2D().outlinedCircle(GLOBAL_RENDER_STACK, 0, 0, element.radius(), element.color(), 1);
        } else {
            ThinGL.renderer2D().filledCircle(GLOBAL_RENDER_STACK, 0, 0, element.radius(), element.color());
        }

        if (this.color.color().toRGBA() != Color.WHITE.toRGBA()) {
            if (element.outlineOnly()) {
                ThinGL.renderer2D().outlinedCircle(GLOBAL_RENDER_STACK, 0, 0, element.radius(), this.color.color(), 1);
            } else {
                ThinGL.renderer2D().filledCircle(GLOBAL_RENDER_STACK, 0, 0, element.radius(), this.color.color());
            }
        }
    }
}
