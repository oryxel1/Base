...
public class PreviewRenderer extends ThinGLElementRenderer<Preview> {
    ...
    private void renderBackground() {
        if (element.background() != null) {
            ...
            ThinGLUtils.blurRectangle(0, 0, Math.max(1920, ThinGL.windowInterface().getFramebufferWidth()), Math.max(ThinGL.windowInterface().getFramebufferHeight(), 1080), Math.round(9 * this.globalFade.getValue())); // Very nice blur thanks to ThinGL.
