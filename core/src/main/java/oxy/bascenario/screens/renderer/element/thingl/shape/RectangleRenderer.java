...
public class RectangleRenderer extends ThinGLElementRenderer<Rectangle> {
    ...
    protected void renderThinGL() {
        if (element.outlineOnly()) {
            ThinGL.renderer2D().outlinedRectangle(GLOBAL_RENDER_STACK, 0, 0, element.width(), element.height(), element.color(), 1);
        } else {
            ThinGL.renderer2D().filledRectangle(GLOBAL_RENDER_STACK, 0, 0, element.width(), element.height(), element.color());
        }

        if (this.color.color().toRGBA() != Color.WHITE.toRGBA()) {
            if (element.outlineOnly()) {
                ThinGL.renderer2D().outlinedRectangle(GLOBAL_RENDER_STACK, 0, 0, element.width(), element.height(), this.color.color(), 1);
            } else {
                ThinGL.renderer2D().filledRectangle(GLOBAL_RENDER_STACK, 0, 0, element.width(), element.height(), this.color.color());
            }
        }
    }
}
