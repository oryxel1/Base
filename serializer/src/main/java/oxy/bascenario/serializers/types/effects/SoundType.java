...
public class SoundType implements Type<Sound> {
    ...
    public JsonElement write(Sound sound) {
        ...
        object.add("file", Types.FILE_INFO_TYPE.write(sound.file()));
    ...
    public Sound read(JsonElement element) {
        ...
        return new Sound(object.get("id").getAsInt(), Types.FILE_INFO_TYPE.read(object.get("file")), object.get("max-volume").getAsFloat(), object.get("loop").getAsBoolean());
    }

    @Override
    public void write(Sound sound, ByteBuf buf) {
        buf.writeInt(sound.id());
        Types.FILE_INFO_TYPE.write(sound.file(), buf);
    ...
    public Sound read(ByteBuf buf) {
        return new Sound(buf.readInt(), Types.FILE_INFO_TYPE.read(buf), buf.readFloat(), buf.readBoolean());
    }
}
